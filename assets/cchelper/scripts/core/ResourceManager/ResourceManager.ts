...
import { Logger } from "../../../eshelper/utils/Logger";
import { singleton } from "../../../eshelper/utils/Singleton";
...
export class ResourceManager
{
    ...
    private loadNewSceneRes(sceneNames:string[])
    {
        return new Promise((resolve,reject)=>{
            ...
            // Start load bundles
            ESUtils.ParallelForEach(_bundles,(_key,_finishLoadBundleStep)=>{
                let _bundleName = _key;
                this.pullBundle(_bundleName).then(_bundle=>{
                    /// Start 加载bundle下的文件夹
                    ESUtils.ParallelForEach(this.config.getScenesBundleDirs(sceneNames, _bundleName),(_assetDir,_loadDirStepFinished)=>{
    ...
    private pullBundle(bundleName:string)
    {
        return new Promise((resolve:(bundle:AssetManager.Bundle)=>void,reject)=>{
            ...
            assetManager.loadBundle(bundleName,(_error,_bundle)=>{
                ...
                console.log("已加载资源包:"+_bundle?.name);
                resolve(_bundle!);
            });
        });
    }
